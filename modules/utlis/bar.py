...
async def download_progress_bar(current, total, message, start_time):
    ...
    progress_text = f"Downloaded: {human_readable_size(current)}/{human_readable_size(total)} ({percentage}%)\n"
    ...
    progress_text += create_progress_bar(current, total)
...
async def upload_progress_bar(current, total, message, start_time):
    ...
    progress_text = f"Uploading: {human_readable_size(current)}/{human_readable_size(total)} ({percentage}%)\n"
    progress_text += f"Speed: {human_readable_size(speed)}/s\n"
    progress_text += f"ETA: {format_time(eta)}\n"
    progress_text += f"Elapsed: {format_time(elapsed_time)}\n"
    progress_text += create_progress_bar(current, total)
...
async def ffmpeg_progress_bar(current_seconds, total_duration, processed_size, message, start_time, estimated_size=None):
    ...
    if estimated_size:
        progress_text += f"ES: {human_readable_size(estimated_size)}\n"

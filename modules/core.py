...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, custom_ffmpeg=None):
    ...
    if custom_ffmpeg and custom_ffmpeg.lower() != "skip":
        ...
        try:
            ...
            # Run FFmpeg command with the custom command provided
            process = subprocess.run(
                f'{custom_ffmpeg} -i "{filename}" "{ffmpeg_output_filename}"',
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
            
            # If successful, replace the original file with the processed one

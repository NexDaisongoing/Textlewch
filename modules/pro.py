...
def pro_feature(bot: Client): @bot.on_message(filters.command("pro") & filters.private) async def pro_handler(_, m: Message): await m.reply_text("📥 Please send me a video file or an .mkv document.") file_msg: Message = await bot.listen(m.chat.id)
...
if file_msg.video:
    ...
    local_in = await file_msg.download(file_name=os.path.join(download_dir, f"input_{m.message_id}{ext}"))
